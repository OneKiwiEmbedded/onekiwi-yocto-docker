...
Example:
  ...
  $0 onekiwi-rzv2l      # set board onekiwi-rzv2l
  $0 smarc-rzv2l        # set board smarc-rzv2l
EOF
}
...
main() {
    ...
    case "${1:-build}" in
        ...
        clean|cleanall|cleansstate|configure|compile|package|fetch)
            ...
            ;;
        smarc-rzv2l)
            CONFIG="cp ../meta-renesas/meta-rzv2l/docs/template/conf/smarc-rzv2l/*.conf conf/"
            START_ECHO="[*] ${CONFIG}"
            RUN_CMD="${CONFIG}"
            END_ECHO="[✓] ${CONFIG}"
            ;;
        onekiwi-rzv2l)
            CONFIG="cp ../meta-renesas/meta-rzv2l/docs/template/conf/onekiwi-rzv2l/*.conf conf/"
            START_ECHO="[*] ${CONFIG}"
            RUN_CMD="${CONFIG}"
            END_ECHO="[✓] ${CONFIG}"
            ;;
